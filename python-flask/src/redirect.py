# Handles public redirects, password checks, and click logging for Flask RedirectIQ.
...
def password_prompt(slug):
    ...
    encoded_slug = escape(slug, quote=True)
    html = f"""<!doctype html>
<html lang="en">
  <head>
    <meta charset="utf-8" />
...
def redirect_to_destination(slug):
    ...
    if link.get("password_hash") and not has_valid_session(link["id"], visitor_token):
        return redirect(f"/password-prompt/{slug}", code=302)
    ...
    ip_address = normalize_ip(request.headers.get("X-Forwarded-For", request.remote_addr or ""))
